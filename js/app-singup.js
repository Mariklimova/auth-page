...
const confirmpwd = document.querySelector('.pwd-2');
...
btn.addEventListener('click', () => {
    try {
        ...
        if (confirmpwd.value !== pwd.value) throw new Error('password not confirmed')
        alert('You are successfully logged in to the system')
        mail.value = '';
        pwd.value = '';
        confirmpwd.value = '';
    } catch (error) {
